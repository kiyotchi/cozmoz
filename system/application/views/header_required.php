...
$output      = array();
$output_mode = config_item('final_output_mode');
$output_carrier = config_item('final_output_carrier');

if ( ($output_carrier === 'au' || $output_carrier === 'softbank') || ($output_mode === 'pc' || $output_mode === 'sp') || ( $output_mode === 'mb' && $this->is_login ) )
{
//	$output[] = '<meta http-equiv="Content-Type" content="text/html; charset=' . (( ! $this->is_login && $output_mode === 'mb') ? 'Shift_JIS' : 'UTF-8') . '" />';
//	$output[] = '<meta http-equiv="Content-Style-Type" content="text/css; charset=' . (( ! $this->is_login && $output_mode === 'mb') ? 'Shift_JIS' : 'UTF-8') . '" />';
//	$output[] = '<meta http-equiv="Content-Script-Type" content="text/javascript; charset=' . (( ! $this->is_login && $output_mode === 'mb') ? 'Shift_JIS' : 'UTF-8') . '" />';
...
//if($this->is_login) {
$output[] = write_favicon(TRUE);

if (!isset($this->is_preview) || $this->is_preview === FALSE)
{
	// create title
	// If Meta-Title is exists, use this as Page-Title.
	$page_title = (isset($meta_title) && !empty($meta_title)) ? $meta_title : $page_title;
	...
	$output[] = '<title>' . prep_str(sprintf(SEEZOO_TITLE_FORMAT, $detail_title . $page_title, $this->site_data->site_title)) . '</title>';

	if (isset($meta_description) && $meta_description != '')
	{
		$output[] = '<meta name="description" content="' . prep_str($meta_description) . '" />';
	}
	if (isset($meta_keyword) && $meta_keyword != '')
	{
		$output[] = '<meta name="keywords" content="' . prep_str($meta_keyword) . '" />';
	}
	...
	if ($this->is_login === TRUE)
	{
		...
		if ($this->cms_mode === FALSE)
		{
			$output[] = flint_execute('segment');
			// Do you want to use jQuery?
			if ( isset($this->site_data->use_jquery) && $this->site_data->use_jquery > 0 )
			{
		...
		{
			$output[] = flint_execute($this->page_id);
		...
		$output[] = 'var SZ_PAGE_ID = ' . $this->page_id . ';';
		$output[] = 'var IS_EDIT = ' . (($this->edit_mode === 'EDIT_SELF') ? 'true' : 'false') . ';';
		if ($this->edit_mode != 'EDIT_SELF')
		{
...
else
{
	...
	{
		$output[] = '<script type="text/javascript" src="' . get_base_link() . 'flint/flint_config/preview" charset="UTF-8"></script>';
	}
	else
	{
		$output[] = '<script type="text/javascript" src="' . get_base_link() . 'flint/flint_config/view" charset="UTF-8"></script>';
	}
...
if (isset($this->is_advance_css) && $this->is_advance_css === TRUE)
{
	$output[] = '<link rel="stylesheet" type="text/css" href="' . get_base_link() . 'page/advance_css/' . $this->template_id . '" id="sz_advance_css_val_link" />';
}
//}
echo implode("\n", $output) . "\n";
