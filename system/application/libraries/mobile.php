 ...
 * @author Y.Paku Bikebros UserAgent用
 *
 */
class Mobile
{
	...
	{
		//Softbank系キャリア判別
		if (preg_match('/^(J\-PHONE|Vodafone|MOT\-[CV]|SoftBank)/i', $this->_agent)) {
			$this->_mobile_carrier = 'softbank';
			$this->_mobile = true;
		//Docomo系キャリア判別
		} elseif (preg_match('/^DoCoMo/i', $this->_agent)) {
			$this->_mobile_carrier = 'docomo';
			$this->_mobile = true;
		//AU系キャリア判別
		} elseif (preg_match('/^KDDI\-/i', $this->_agent)||preg_match('/UP\.Browser/i',$this->_agent)){
			$this->_mobile_carrier = 'au';
			$this->_mobile = true;
		}else {
		//その他からのアクセス
			$this->_mobile_carrier = '';
			$this->_mobile = false;
		}
	...
	{
		$guid = '';
		if(!$this->_mobile) return $guid;

		switch ( $this->_mobile_carrier )
		{
			case 'softbank':
	...
	/**
	 * ユーザーエイジェントからデバイス名を取得
	 */
	public function getDeviceName()
	{
		if($this->is_docomo()){
		    //DoCoMoの場合
			if(preg_match("/^DoCoMo\/2\.0\s([0-9a-zA-Z]+)/",$this->_agent,$match)) {
				return $match[1];
			} else {
				return '';
			}
		} else if ($this->is_softbank()){
		    //Softbankの場合
		    $device = $_SERVER{'HTTP_X_JPHONE_MSNAME'};
		    return $device;
		} else if ($this->is_au()){
		    //auの場合
			if(preg_match("/[KDDI-]([0-9A-Z]{4})/",$this->_agent,$match)) {
				return $match[1];
			// auHDML,Tu-Ka機種判別
			} elseif(preg_match("/^UP\.Browser.*-([0-9A-Z]){4}/",$this->_agent,$match)) {
				return $match[1];
			} else {
				return '';
			}
